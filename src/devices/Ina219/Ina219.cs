...
namespace Iot.Device.ina219
{
    public class Ina219 : IDisposable
    {
        ...
        private byte _ina219Address = (0x40);    // 1000000 (A0+A1=GND)
        private UInt16 _ina219_calValue = 0x1000;
        private float _ina219_currentDivider_mA = 20;
        private float _ina219_powerDivider_mW = 1;
        private bool _enable_low_current_nfilter = true;
        private float _ncurrent_filter_threshold = -0.3f;
        private CurrentUnit _currentUnit = CurrentUnit.uA;
        ...
        /// <summary>
        /// Truncates current read to zero for small negative current 
        /// <seealso cref="Current_NFilter_Threshold"/>
        /// </summary>
        public bool Low_Current_NFilter_Enable { get { return _enable_low_current_nfilter; } set { _enable_low_current_nfilter = value; } }

        /// <summary>
        /// Values between this number and zero will be considered zero
        /// </summary>
        public float Current_NFilter_Threshold { get { return _ncurrent_filter_threshold; } set { _ncurrent_filter_threshold = value; } }

        public CurrentUnit UnitCurrent { get { return _currentUnit; } set { _currentUnit = value; } }
        ...
        {
            Span<byte> data=BitConverter.GetBytes(value);

            //.WriteRegister(_ina219Address, INA219_REG_CONFIG, value);
            _i2cDevice.WriteByte(INA219_REG_CONFIG);
        ...
        public UInt16 ReadConfiguration()
        {
            ...
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_CONFIG);
            //UInt16 val = (UInt16)_fti2c.ReadRegister(_ina219Address);
        ...
        {
            // _fti2c.WriteRegister(_ina219Address, INA219_REG_CALIBRATION, value);
            _i2cDevice.WriteByte(INA219_REG_CALIBRATION);
            Span<byte> data = BitConverter.GetBytes(value);
            _i2cDevice.Write(data);
        }
        ...
        {
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_CALIBRATION);
            //UInt16 val = (UInt16)_fti2c.ReadRegister(_ina219Address);
        ...
        {
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_SHUNTVOLTAGE);
            //Int16 volts_shunt_reg = _fti2c.ReadRegister(_ina219Address);

            UInt16 value = 0;

            _i2cDevice.WriteByte(INA219_REG_SHUNTVOLTAGE);
            Span<byte> data = BitConverter.GetBytes(value);
            _i2cDevice.Read(data);

            // Shift to the right 3 to drop CNVR and OVF and multiply by LSB
            float volts_shunt = value * 0.01f;
        ...
        {
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_BUSVOLTAGE);
            //Int16 volts_reg = _fti2c.ReadRegister(_ina219Address);

            Int16 volts_reg = 0;

            _i2cDevice.WriteByte(INA219_REG_BUSVOLTAGE);
            Span<byte> data = BitConverter.GetBytes(volts_reg);
            _i2cDevice.Read(data);
        ...
        /// <summary>
        /// Gets the current register value and returns it in mA
        /// <seealso cref="Low_Current_NFilter_Enable"/>
        /// </summary>
        /// <returns></returns>
        public float GetCurrent()
        {
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_CURRENT);
            //Int16 current_reg = _fti2c.ReadRegister(_ina219Address);\

            Int16 current_reg = 0;

            _i2cDevice.WriteByte(INA219_REG_CURRENT);
            Span<byte> data = BitConverter.GetBytes(current_reg);
            _i2cDevice.Read(data);

            float current = current_reg / _ina219_currentDivider_mA;

            if (Low_Current_NFilter_Enable)
            {
                if (current < 0 && current > Current_NFilter_Threshold)
                    current = 0.0f;
            }

            if (UnitCurrent == CurrentUnit.uA)
                current *= 1000;
        ...
        {
            //_fti2c.RegisterPointerSet(_ina219Address, FTI2C.RW.WRITE, INA219_REG_POWER);
            //Int16 reg = _fti2c.ReadRegister(_ina219Address);
            ...
            float power = value / _ina219_powerDivider_mW;
